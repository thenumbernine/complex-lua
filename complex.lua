...
--			self.init(re.value)
	...
	function complex:isa(x)
		...
		local xt = ffi.typeof(x)
		return xt == complexFloatType
			or xt == complexDoubleType
	end
end

function complex.__eq(a,b)
	if not complex:isa(a) and not complex:isa(b) then return false end
	a = complex(a)
	b = complex(b)
